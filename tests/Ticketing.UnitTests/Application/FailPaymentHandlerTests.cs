...
public class FailPaymentHandlerTests
{
    private readonly Mock<IPaymentRepository> _paymentMock;
    private readonly Mock<IUnitOfWork> _uowMock;
    private readonly FailPaymentHandler _sut;

    public FailPaymentHandlerTests()
    {
        _paymentMock = new Mock<IPaymentRepository>();
        _uowMock = new Mock<IUnitOfWork>();
        var loggerMock = new Mock<ILogger<FailPaymentHandler>>();
        _sut = new FailPaymentHandler(_paymentMock.Object, _uowMock.Object, loggerMock.Object);
    }

    [Fact]
    public async Task Handle_ShouldFailPayment()
    {
        ...
        await _sut.Handle(new FailPaymentCommand(payment.Id), CancellationToken.None);
    ...
    public async Task Handle_WhenPaymentNotFound_ShouldThrow()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            _sut.Handle(new FailPaymentCommand(paymentId), CancellationToken.None));
    }
}
